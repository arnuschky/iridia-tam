...
#define DM_PACKET_ACKNOWLEDGED 0x01
#define DM_BROADCAST_PACKET 0x02
...
#define DM_REMOTE_AT_REQUEST 0x19
#define ZB_TX_REQUEST 0x10
#define DM_TX_REQUEST 0x10
#define ZB_EXPLICIT_TX_REQUEST 0x11
#define DM_EXPLICIT_TX_REQUEST 0x11
...
#define DM_BROADCAST_RADIUS_MAX_HOPS 0

#define ZB_TX_UNICAST 0
#define ZB_TX_BROADCAST 8
#define DM_TX_BROADCAST 8
...
public:
	...
	/**
	 * Call with instance of DMTxStatusResponse class only if getApiId() == DM_TX_STATUS_RESPONSE
	 * to populate response
	 */
        void getDMTxStatusResponse(XBeeResponse &response);
        /**
         * Call with instance of DMRxResponse class only if getApiId() == DM_RX_RESPONSE
         * to populate response
         */
...
#ifdef SERIES_DIGIMESH
/**
 * Represents a Series DM TX status packet
 */
class DMTxStatusResponse : public FrameIdResponse {
        public:
                DMTxStatusResponse();
                uint8_t getTxRetryCount();
                uint8_t getDeliveryStatus();
...
/**
 * Represents a Series DM RX packet
 */
class DMRxResponse : public RxDataResponse {
public:
        DMRxResponse();
        XBeeAddress64& getRemoteAddress64();
        uint8_t getOption();
 ...
 * exceeds the maximum.
 */
class DMTxRequest : public PayloadRequest {
public:
        /**
         * Creates a unicast DMTxRequest with the ACK option and DEFAULT_FRAME_ID
         */
        DMTxRequest(XBeeAddress64 &addr64, uint8_t *payload, uint8_t payloadLength);
        DMTxRequest(XBeeAddress64 &addr64, uint8_t broadcastRadius, uint8_t option, uint8_t *payload, uint8_t payloadLength, uint8_t frameId);
        /**
         * Creates a default instance of this class.  At a minimum you must specify
         * a payload, payload length and a destination address before sending this request.
         */
