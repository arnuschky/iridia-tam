...
function(SETUP_ARDUINO_SIZE_SCRIPT OUTPUT_VAR)
    ...
    if(SIZE_ROW MATCHES \"[ \\t]*([0-9]+)[ \\t]*([0-9]+)[ \\t]*([0-9]+)[ \\t]*([0-9]+)[ \\t]*([0-9a-fA-F]+).*\")
        message(\"\n  text size : \${CMAKE_MATCH_1}\t\tcode (flash) memory\")
        message(\"  data size : \${CMAKE_MATCH_2}\t\tinitialised data (in RAM - includes strings etc)\")
        message(\"  bss size  : \${CMAKE_MATCH_3}\t\tnon-initialised data (also in RAM)\")
        message(\"  total size: \${CMAKE_MATCH_4}\t\ttotal elf size\")
        message(\"\n  All values given in decimals and bytes.\")
        message(\"  Total RAM usage = (data + bss).\n\")
        message(\"  Note that this does NOT include dynamic\")
        message(\"  memory allocated from the heap at run time.\n\")
    endif()")
