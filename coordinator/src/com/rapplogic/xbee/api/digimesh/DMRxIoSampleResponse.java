...
/**
 * Series 1 XBee with Digimesh.  Represents an I/O Sample resposne sent from a remote radio.
 * Provides access to the XBee's 4 Analog (0-4), 11 Digital (0-7,10-12), and 1 Supply Voltage pins
 * <p/>
 * Note: Series 1 with Digimesh does not support multiple samples (IT) per packet
 * <p/>
 * @author Arne Brutschy
 */
public class DMRxIoSampleResponse extends DMRxBaseResponse implements NoRequestResponse {
	
	private final static Logger log = Logger.getLogger(DMRxIoSampleResponse.class);
	...
	public DMRxIoSampleResponse() {
		
	}

	public static DMRxIoSampleResponse parseIsSample(AtCommandResponse response) throws IOException {
		...
		DMRxIoSampleResponse sample = new DMRxIoSampleResponse();
		sample.parse(in);
	...
	public void parse(IIntArrayInputStream ps) throws IOException {
		...
		int size = ps.read("DM RX IO Sample Size");
		
		if (size != 1) {
			throw new XBeeParseException("Sample size is not supported if > 1 for DM I/O");
		}
		
		this.setDigitalChannelMaskMsb(ps.read("DM RX IO Sample Digital Mask 1"));
		this.setDigitalChannelMaskLsb(ps.read("DM RX IO Sample Digital Mask 2"));
		this.setAnalogChannelMask(ps.read("DM RX IO Sample Analog Channel Mask"));
		...
		if (this.containsDigital()) {
			...
			this.setDioMsb(ps.read("DM RX IO DIO MSB"));
			this.setDioLsb(ps.read("DM RX IO DIO LSB"));
		} else {
