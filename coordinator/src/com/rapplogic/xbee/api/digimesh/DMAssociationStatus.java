...
/**
 * Series 1 XBee with Digimesh.
 * TODO: fix this?
 * @author Arne Brutschy
 */
public enum DMAssociationStatus
{
	...
	private static final Map<Integer,DMAssociationStatus> lookup = new HashMap<Integer,DMAssociationStatus>();
	
	static {
		for(DMAssociationStatus s : EnumSet.allOf(DMAssociationStatus.class)) {
			lookup.put(s.getValue(), s);
		}
	}
	
	public static DMAssociationStatus get(int value) {
		return lookup.get(value); 
	}
	
	public static DMAssociationStatus get(AtCommandResponse response) {
		return DMAssociationStatus.get(response.getValue()[0]);
	}
	
    private final int value;
    private final String description;
    
    DMAssociationStatus(int value, String description) {
        this.value = value;
