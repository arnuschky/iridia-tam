...
import com.rapplogic.xbee.api.digimesh.DMAssociationStatus;
 ...
 */
public class DMApiAtExample
{

	private final static Logger log = Logger.getLogger(DMApiAtExample.class);
	
	private XBee xbee = new XBee();
	
	public DMApiAtExample() throws XBeeException {
		try {
			...
			log.debug("SH is " + ByteUtils.toBase16(((AtCommandResponse)xbee.sendSynchronous(new AtCommand("SH"))).getValue()));
			log.debug("SL is " + ByteUtils.toBase16(((AtCommandResponse)xbee.sendSynchronous(new AtCommand("SL"))).getValue()));
...
//			this.configureEndDevice(xbee);
	...
	private void associationStatus(XBee xbee) throws XBeeException {
		...
		log.debug("Association Status is " + DMAssociationStatus.get(response));
	}
	...
	private void configureEndDevice(XBee xbee) throws XBeeException {
		...
		response = xbee.sendSynchronous(new AtCommand("RE"));
		log.debug("RE is " + response);

		// set PAN id to arbitrary value
		response = xbee.sendSynchronous(new AtCommand("ID", new int[] {0x1a, 0xaa}));
		log.debug("ID is " + response);
		
		// set NI -- can be any arbitrary sequence of chars
		response = xbee.sendSynchronous(new AtCommand("NI", new int[] {'E','N','D','_','D','E','V','I','C','E','_','2' }));
		log.debug("NI is " + response);

		// set API mode to 2.  factory setting is 1
		response = xbee.sendSynchronous(new AtCommand("AP", 2));
		log.debug("AP is " + response);			

		// save to settings to survive power cycle
		response = xbee.sendSynchronous(new AtCommand("WR"));
		log.debug("WR is " + response);			

		// software reset
		response = xbee.sendSynchronous(new AtCommand("FR"));
	...
	private void configureCoordinator(XBee xbee) throws XBeeException {			
		...
		// set NI
		response = xbee.sendSynchronous(new AtCommand("NI", new int[] {'C','O','O','R','D','I','N','A','T','O','R' }));
		log.debug("NI is " + response);
	...
	private void configureIOSamples(XBee xbee) throws XBeeException {			
		...
		response = xbee.sendSynchronous(new AtCommand("IR", new int[] {0x27, 0x10}));
		log.debug("IR is " + response);

		// set pin 20 to monitor digital input
		response = xbee.sendSynchronous(new AtCommand("DO", 0x3));
		log.debug("DO is " + response);

		// set pin 19 to monitor analog input
		response = xbee.sendSynchronous(new AtCommand("D1", 0x2));
		log.debug("D1 is " + response);
		
		// set pin 18 to monitor analog input
		response = xbee.sendSynchronous(new AtCommand("D2", 0x2));
		log.debug("D2 is " + response);
		
		// set pin 16 to monitor digital input
		response = xbee.sendSynchronous(new AtCommand("D6", 0x3));
