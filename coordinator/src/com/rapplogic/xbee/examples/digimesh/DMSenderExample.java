 ...
 */
public class DMSenderExample
{

	private final static Logger log = Logger.getLogger(DMSenderExample.class);
	
	private DMSenderExample() throws XBeeException {
...
//			XBeeAddress64 addr64 = new XBeeAddress64(0, 0x13, 0xa2, 0, 0x40, 0x0a, 0x3e, 0x02);
            ...
            DMTxRequest request = new DMTxRequest(addr64, payload);
			...
			while (true) {
				...
				try {
					DMTxStatusResponse response = (DMTxStatusResponse) xbee.sendSynchronous(request, 10000);
					...
					if (response.getDeliveryStatus() == DMTxStatusResponse.DeliveryStatus.SUCCESS) {
