...
import com.rapplogic.xbee.api.digimesh.DMForceSampleRequest;
 ...
 */
public class DMForceSampleExample
{

	private final static Logger log = Logger.getLogger(DMForceSampleExample.class);
	
	private DMForceSampleExample() throws Exception {
		XBee xbee = new XBee();		
		...
		try {			
			...
			while (true) {
				...
				XBeeRequest request = new DMForceSampleRequest(addr64);
				...
				try {
					...
					if (remoteAt.isOk())  {
                        ...
                        DMRxIoSampleResponse ioSample = DMRxIoSampleResponse.parseIsSample(remoteAt);
