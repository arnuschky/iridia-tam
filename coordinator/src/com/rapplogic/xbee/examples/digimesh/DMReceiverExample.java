 ...
 */
public class DMReceiverExample
{

	private final static Logger log = Logger.getLogger(DMReceiverExample.class);
	
	private DMReceiverExample() throws Exception {
		XBee xbee = new XBee();		
			...
			while (true) {
				...
				try {
					...
					if (response.getApiId() == ApiId.DM_RX_RESPONSE) {
                        ...
                        DMRxResponse rx = (DMRxResponse) response;
                        ...
                        AtCommandResponse atResponse2 = (AtCommandResponse)atResponse;
                        log.info("at command: "+atResponse2.getCommand());
